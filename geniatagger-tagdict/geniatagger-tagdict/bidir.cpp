...
static wordClusterPath * pwordclusterpath = new wordClusterPath();
static wordListFeatures * pwordlistfeature = new wordListFeatures();
         ...
         const string & pos_right1, const string & pos_right2)
{
  ...
  //gen word cluster feature
#if 0
  std::vector<std::string> clusterFeature;
  std::vector<std::string> clusterPreFeature;
  std::vector<std::string> clusterNextFeature;
  std::vector<std::string>::iterator it;
  pwordclusterpath->loadWord2PathDict();
  pwordclusterpath->genClusterFeature(str, clusterFeature);
  for(it = clusterFeature.begin();it != clusterFeature.end();++it)
  {
  	sample.features.push_back(*it);
  }
  if(i > 0)
  {
  	std::string tmp = vt[i-1].str;
	pwordclusterpath->genPreClusterFeature(tmp, clusterPreFeature);
  	for(it = clusterPreFeature.begin();it != clusterPreFeature.end();++it)
  	{
  	  sample.features.push_back(*it);
    }
  }
  ...
  {
    std::string tmp = vt[i+1].str;
	pwordclusterpath->genNextClusterFeature(tmp, clusterNextFeature);
  	for(it = clusterNextFeature.begin();it != clusterNextFeature.end();++it)
  	{
  	  sample.features.push_back(*it);
    }
...
#endif
  pwordlistfeature->genPosTagFeature(str, sample.rvfeatures);
  //pwordlistfeature->genMetaphoneTagFeature(str, sample.rvfeatures);
  ...
  std::cout<<"sample.label : "<<sample.label<<std::endl;

  std::cout<<"sample.features : "<<std::endl;

  for(vector<string>::const_iterator j = sample.features.begin(); j != sample.features.end(); j++) {
    cout << *j <<std::endl;
  }

  for(vector<pair<string, double> >::const_iterator j = sample.rvfeatures.begin(); j != sample.rvfeatures.end(); j++) {
    std::cout<<"feature : "<<j->first<<"--> value : "<<j->second<<std::endl;
  }
  cout << endl << endl;
...
bidir_train(const vector<Sentence> & vs, int para)
{
  ...
  //for (int t = 0; t < 16; t++) {
   // if (t != 15 && t != 0) continue;
    ...
    m.train(train, 2, 0, 1);
    char buf[1000];
    sprintf(buf, "./models_posCikuu/model.bidir.%d", t);
    m.save_to_file(buf);
...
/*void bidir_postag_train is added by wukun*/

void split(string s,vector<string> & vstr)
{
...
void split(string s,list<string> & vstr)
{
	char * str = const_cast<char *>(s.c_str());
	char *pch = strtok(str," ");
	while(pch != NULL)
...
void bidir_pos_train(string filename,int para)
{
  ...
  while(getline(fin,s))
  {
	...
	split(s,lt);
	vector<Token> vt;
	for(list<string>::const_iterator it= lt.begin();it != lt.end();it++)
	{
		string s = *it;
        //std::cout<<s<<std::endl;
		vector<string> vstr;
		...
		vt.push_back(Token(vstr[0],vstr[1]));
	}
	vs.push_back(vt);
	lt.clear();
    vt.clear();
